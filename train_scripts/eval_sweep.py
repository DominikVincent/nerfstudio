...
parser.add_argument("--eval_config", help="The nesf eval config", type=str, default="/data/vision/polina/projects/wmh/dhollidt/documents/nerf/data/nesf_test_config_test.json")
...
def ns_eval(config_path, output_path):
    command = f"ns-eval --load-config {config_path} --output-path {output_path} --use-wandb"
...
def eval_run(path: Path, eval_set: Union[None, Path]=None, wandb_config = None, gpu_index: int = 0):
    ...
    ns_eval(out_path, path / "auto_eval.json")
...
def main(project_name: str, sweep_id: str, eval_config: Union[None, Path]=None):
    ...
    for i, run in enumerate(runs):
        ...
        print(path)
        result = pool.apply_async(eval_run, args=(path, eval_config, run.config, gpu_index))
        results.append(result)
        # eval_run(path, eval_config, run.config, gpu_index)
